...
def table_data(frame: pd.DataFrame) -> dict[str, object]:
    cls = pd.read_csv(CLASSIFICATION_METRICS, encoding="utf-8-sig")
    reg = pd.read_csv(REGRESSION_METRICS, encoding="utf-8-sig")
    rank = pd.read_csv(RANK_METRICS, encoding="utf-8-sig")
    return {
        "rows": len(frame),
...
def add_table(doc: Document, headers: Sequence[str], rows: Sequence[Sequence[object]]) -> None:
...
def markdown_report(stats: dict[str, object]) -> str:
    cls = stats["classification"]
    reg = stats["regression"]
    best_cls = cls.sort_values("roc_auc", ascending=False).iloc[0]
    best_reg = reg.sort_values("r2", ascending=False).iloc[0]
...
ROC-AUC 기준 최상위 모델은 {experiment_label(best_cls['experiment'])} / {model_label(best_cls['model'])}이며 ROC-AUC는 {best_cls['roc_auc']:.3f}, Accuracy는 {best_cls['accuracy']:.3f}, F1은 {best_cls['f1']:.3f}이다. Accuracy 기준으로는 metadata / Random Forest가 0.824로 가장 높았다.
