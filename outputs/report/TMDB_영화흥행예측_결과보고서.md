...
ROC-AUC 기준 최상위 모델은 메타+제목 / RF이며 ROC-AUC는 0.822, Accuracy는 0.809, F1은 0.468이다. Accuracy 기준으로는 metadata / Random Forest가 0.824로 가장 높았다.

## 6. 회귀 결과

R2 기준 최상위 회귀 모델은 전체 / Linear이며 R2는 0.211, RMSE는 2,250,648명이다. 관객 수 분포가 블록버스터 영화에 크게 치우쳐 있어 회귀 예측의 설명력은 제한적이었다.
